...
function saveHistory(h) { try { localStorage.setItem(HISTORY_KEY, JSON.stringify(h.slice(-50))) } catch { /* storage may be unavailable */ } }

// Phase 4 moved the Anthropic key off the client. The component no longer
// resolves a key — askAssistant() routes through /api/ai/chat which uses
// the operator's server-side key.
export default function AIAssistant({ open, onClose, context, lang, knowledgeBase, contacts, deals, tickets }) {
  ...
  const send = async (text) => {
    ...
    try {
      const reply = await askAssistant({ question: msg, context: fullContext, history: messages.slice(-10) })
      const updated = [...next, { role: 'assistant', content: reply }]
      setMessages(updated); saveHistory(updated)
    } catch (err) {
      const fallback = isRTL
        ? 'حدث خطأ أثناء الاتصال بالذكاء الاصطناعي. قد لا تكون ميزة AI مفعلة لهذه العيادة — تواصل مع المشغل.'
        : 'Failed to reach the AI service. AI may not be enabled for this clinic — contact the operator.'
  ...
  return (
    <div style={{ position:'fixed', bottom:24, [isRTL?'left':'right']:24, width:400, maxWidth:'calc(100vw - 48px)', height:540, maxHeight:'calc(100vh - 100px)', background:'#101422', borderRadius:16, border:'1px solid rgba(20,184,166,0.12)', boxShadow:'0 16px 48px rgba(0,0,0,.5), 0 0 30px rgba(20,184,166,0.05)', zIndex:1800, display:'flex', flexDirection:'column', overflow:'hidden', direction:isRTL?'rtl':'ltr' }}>
      {/* Header */}
      <div style={{ padding:'12px 16px', borderBottom:`1px solid ${C.border}`, display:'flex', alignItems:'center', gap:10 }}>
        ...
        <div style={{ flex:1 }}><div style={{ fontSize:14, fontWeight:700, color:C.text }}>SupCod3 AI</div><div style={{ fontSize:10, color:C.textMuted }}>{isRTL?'مساعد CRM الذكي':'CRM Assistant'}</div></div>
      ...
      {/* ── Chat interface ─────────────────────────────────
         AI is operator-managed. If the proxy returns an error
         (no key configured server-side), it surfaces via the
         inline error banner below the messages — no client-
         side "configure your key" UI is needed. */}
      {/* Messages */}
      <div style={{ flex:1, overflow:'auto', padding:'12px 16px', display:'flex', flexDirection:'column', gap:10 }}>
        ...
        {messages.map((msg,i) => (
          <div key={i} style={{ display:'flex', justifyContent:msg.role==='user'?'flex-end':'flex-start' }}>
            <div style={{ maxWidth:'85%', padding:'10px 14px', borderRadius:12, background:msg.role==='user'?'linear-gradient(135deg, #14B8A6, #4DA6FF)':'#0C0E1A', color:msg.role==='user'?'#080c14':'#e2e8f0', border:msg.role==='user'?'none':'1px solid rgba(20,184,166,0.12)', fontSize:13, lineHeight:1.5, whiteSpace:'pre-wrap' }}>{msg.content}</div>
          </div>
        ))}
        ...
        {error && <div style={{ padding:'8px 12px', borderRadius:8, background:'rgba(239,68,68,0.08)', border:'1px solid rgba(239,68,68,0.15)', fontSize:11, color:'#ef4444' }}>{error}</div>}
        <div ref={endRef} />
      </div>

      {/* Input */}
      <div style={{ padding:'10px 14px', borderTop:`1px solid ${C.border}`, display:'flex', gap:8 }}>
        <input value={input} onChange={e=>setInput(e.target.value)} onKeyDown={e=>e.key==='Enter'&&send()} placeholder={isRTL?'اسأل SupCod3 AI...':'Ask SupCod3 AI...'} style={{ flex:1, padding:'9px 12px', borderRadius:8, border:`1px solid ${C.border}`, fontSize:13, color:C.text, fontFamily:'inherit', outline:'none', direction:isRTL?'rtl':'ltr', background:C.bg }} />
