  ...
  return (
    <div style={{ direction:dir }}>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fit,minmax(160px,1fr))', gap:12, marginBottom:24 }}>
        {[
          { label:isRTL?'إجمالي الإيرادات':'Total Revenue', value:totalRevenue, color:C.success, bg:C.successBg },
          { label:isRTL?'معلق':'Total Pending', value:totalPending, color:C.warning, bg:C.warningBg },
          { label:isRTL?'متأخر':'Overdue', value:totalOverdue, color:C.danger, bg:C.dangerBg },
      ...
      <div className="fade-in" key={tab}>
        ...
        {tab === 'overview' && (
          <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:20 }}>
            ...
            <div style={{ ...card, padding:20 }}>
              ...
              {topContacts.length === 0 ? <p style={{ fontSize:13, color:C.textMuted, textAlign:'center', padding:16 }}>{isRTL?'لا توجد بيانات':'No data'}</p> : topContacts.map(([name,val],i) => {
                const maxVal = topContacts[0]?.[1] || 1
                return (
                  <div key={name} style={{ marginBottom:12 }}>
                    <div style={{ display:'flex', justifyContent:'space-between', fontSize:13, marginBottom:4 }}><span style={{ color:C.text, fontWeight:500 }}>{name}</span><span style={{ fontWeight:600, color:C.text }}>{fmtMoney(val,cur)}</span></div>
          ...
          <div style={{ ...card, overflow:'hidden' }}>
            <table style={{ width:'100%', borderCollapse:'collapse', fontSize:14 }}>
              <thead><tr style={{ background:C.bg, borderBottom:`1px solid ${C.border}` }}>
                {[isRTL?'جهة الاتصال':'Contact', isRTL?'المبلغ':'Amount', isRTL?'الطريقة':'Method', isRTL?'التاريخ':'Date', isRTL?'الوصف':'Description'].map((h,i)=><th key={i} style={thStyle(isRTL)}>{h}</th>)}
              </tr></thead>
              <tbody>{paidPayments.length===0 ? <tr><td colSpan={5} style={{ padding:32, textAlign:'center', color:C.textMuted }}>{isRTL?'لا يوجد دخل':'No income recorded'}</td></tr> : paidPayments.map(p => (
                <tr key={p.id} style={{ borderBottom:`1px solid ${C.border}`, transition:'all 150ms ease' }}>
                  ...
                  <td style={{ padding:'10px 16px', fontWeight:700, color:C.success }}>{fmtMoney(p.amount, p.currency||cur)}</td>
                  <td style={{ padding:'10px 16px', color:C.textSec }}>{PAYMENT_METHODS_LABELS[p.method]||p.method}</td>
                  <td style={{ padding:'10px 16px', color:C.textMuted, fontSize:13 }}>{p.paymentDate||p.date}</td>
        ...
        {tab === 'pending' && (
          <div style={{ ...card, overflow:'hidden' }}>
            <table style={{ width:'100%', borderCollapse:'collapse', fontSize:14 }}>
              <thead><tr style={{ background:C.bg, borderBottom:`1px solid ${C.border}` }}>
                {[isRTL?'جهة الاتصال':'Contact', isRTL?'المبلغ':'Amount', isRTL?'الاستحقاق':'Due Date', isRTL?'الحالة':'Status', isRTL?'الوصف':'Description'].map((h,i)=><th key={i} style={thStyle(isRTL)}>{h}</th>)}
              </tr></thead>
              <tbody>{pendingPayments.length===0 ? <tr><td colSpan={5} style={{ padding:32, textAlign:'center', color:C.textMuted }}>{isRTL?'لا توجد مدفوعات معلقة':'No pending payments'}</td></tr> : pendingPayments.map(p => {
                ...
                return (
                  <tr key={p.id} style={{ borderBottom:`1px solid ${C.border}`, background: isOverdue?`${C.dangerBg}08`:'transparent', transition:'all 150ms ease' }}>
                    <td style={{ padding:'10px 16px', fontWeight:500, color:C.text }}>{p.contactName}</td>
                    <td style={{ padding:'10px 16px', fontWeight:700, color: isOverdue?C.danger:C.warning }}>{fmtMoney(p.amount, p.currency||cur)}</td>
                ...
                <tbody>{expenses.length===0 ? <tr><td colSpan={5} style={{ padding:32, textAlign:'center', color:C.textMuted }}>{isRTL?'لا توجد مصروفات':'No expenses recorded'}</td></tr> : expenses.sort((a,b)=>(b.date||'').localeCompare(a.date||'')).map(e => {
                  ...
                  return (
                    <tr key={e.id} style={{ borderBottom:`1px solid ${C.border}`, transition:'all 150ms ease' }}>
                      ...
                      <td style={{ padding:'10px 16px', fontWeight:700, color:C.danger }}>{fmtMoney(e.amount, e.currency||cur)}</td>
                      <td style={{ padding:'10px 16px', color:C.textMuted, fontSize:13 }}>{e.date}</td>
                      <td style={{ padding:'10px 16px', color:C.textSec, fontSize:13 }}>{e.description||'—'}{e.receipt?` 📎 ${e.receipt}`:''}</td>
