...
import { fetchAllPayments } from '../lib/database'
...
export default function FinancePage({ t, lang, dir, isRTL, contacts, currency, toast, showConfirm }) {
  ...
  const allPayments = payments.map(p => ({
    ...p,
    contactName: contactMap[p.contactId] || '',
          ...
          <div className="fade-in" key={tab}>
              ...
              <div style={{ ...card, overflow: 'hidden' }}>
                <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 14 }}>
                  <thead><tr style={{ background: C.bg, borderBottom: `1px solid ${C.border}` }}>
                    {[isRTL ? 'جهة الاتصال' : 'Contact', isRTL ? 'المبلغ' : 'Amount', isRTL ? 'العملة' : 'Currency', isRTL ? 'الطريقة' : 'Method', isRTL ? 'التاريخ' : 'Date', isRTL ? 'الوصف' : 'Description'].map((h, i) => <th key={i} style={thStyle(isRTL)}>{h}</th>)}
                  </tr></thead>
                  <tbody>{paidPayments.length === 0 ? <tr><td colSpan={6} style={{ padding: 32, textAlign: 'center', color: C.textMuted }}>{isRTL ? 'لا يوجد دخل' : 'No income recorded'}</td></tr> : paidPayments.map(p => (
            ...
            {tab === 'pending' && (
              <div style={{ ...card, overflow: 'hidden' }}>
                <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 14 }}>
                  <thead><tr style={{ background: C.bg, borderBottom: `1px solid ${C.border}` }}>
                    {[isRTL ? 'جهة الاتصال' : 'Contact', isRTL ? 'المبلغ' : 'Amount', isRTL ? 'العملة' : 'Currency', isRTL ? 'الاستحقاق' : 'Due Date', isRTL ? 'الحالة' : 'Status', isRTL ? 'الوصف' : 'Description'].map((h, i) => <th key={i} style={thStyle(isRTL)}>{h}</th>)}
                  </tr></thead>
                  <tbody>{pendingPayments.length === 0 ? <tr><td colSpan={6} style={{ padding: 32, textAlign: 'center', color: C.textMuted }}>{isRTL ? 'لا توجد مدفوعات معلقة' : 'No pending payments'}</td></tr> : pendingPayments.map(p => {
