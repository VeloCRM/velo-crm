  ...
  return (
    <div style={{ direction: dir }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 24 }}>
        <div>
          <h1 style={{ fontSize: 24, fontWeight: 700, color: C.text, margin: 0, fontFamily: 'DM Sans,Inter,sans-serif' }}>{t.integrations}</h1>
      ...
      <div style={{ display: 'flex', gap: 12, marginBottom: 24, flexWrap: 'wrap', alignItems: 'center' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, background: C.white, borderRadius: 8, padding: '8px 12px', border: `1px solid ${C.border}`, flex: 1, maxWidth: 300 }}>
        ...
        <div style={{ display: 'flex', gap: 4 }}>
          {CATEGORIES.map(cat => (
            <button key={cat.id} onClick={() => setFilter(cat.id)}
              style={{ padding: '6px 16px', borderRadius: 6, border: 'none', fontSize: 12, fontWeight: 600, cursor: 'pointer', fontFamily: 'inherit', background: filter === cat.id ? C.primary : C.bg, color: filter === cat.id ? '#fff' : C.textSec, transition: 'all 150ms ease' }}>
              {cat.label}
            </button>
          ))}
        </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(320px, 1fr))', gap: 16 }}>
        {filtered.map(ig => (
          <div key={ig.id} style={{ ...card, padding: 20, display: 'flex', flexDirection: 'column', gap: 16, position: 'relative', transition: 'all 150ms ease' }}>
            {ig.connected && <div style={{ position: 'absolute', top: 16, right: 16, width: 10, height: 10, borderRadius: '50%', background: '#25D366', border: '2px solid #fff', boxShadow: `0 0 0 1px ${C.successBg}` }} />}
            <div style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
              <div style={{ width: 44, height: 44, borderRadius: 8, background: `${ig.color}15`, color: ig.color, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 20, fontWeight: 700, flexShrink: 0 }}>
                {ig.initial}
              </div>
              <div>
                <div style={{ fontSize: 14, fontWeight: 700, color: C.text }}>{ig.name}</div>
                <div style={{ fontSize: 12, color: C.textMuted, textTransform: 'capitalize' }}>{ig.category}</div>
              </div>
            </div>
            <p style={{ fontSize: 13, color: C.textSec, lineHeight: 1.5, margin: 0, flex: 1 }}>{ig.desc}</p>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
              <span style={{ fontSize: 12, fontWeight: 600, padding: '4px 12px', borderRadius: 6, background: ig.connected ? C.successBg : C.bg, color: ig.connected ? C.success : C.textMuted }}>
                {ig.connected ? (t.connected || 'Connected') : (lang === 'ar' ? 'غير متصل' : 'Not connected')}
              </span>
              <button onClick={() => toggleConnection(ig.id)}
                style={ig.connected ? makeBtn('secondary', { fontSize: 12, padding: '4px 12px' }) : makeBtn('primary', { fontSize: 12, padding: '4px 12px' })}>
