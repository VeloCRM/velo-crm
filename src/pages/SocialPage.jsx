...
const PLATFORMS = [
  ...
  { id:'twitter', name:'X (Twitter)', color:'#1F2328', icon:'𝕏', maxChars:280 },
  ...
  return (
    <div style={{ direction:dir }}>
      <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:20 }}>
        <h1 style={{ fontSize:24, fontWeight:700, color:C.text, margin:0 }}>{isRTL?'التواصل الاجتماعي':'Social Media'}</h1>
      </div>
      <div style={{ display:'flex', gap:0, marginBottom:20, borderBottom:`2px solid ${C.border}` }}>
        {tabs.map(tb => (
          <button type="button" key={tb.id} onClick={() => setTab(tb.id)} style={{ padding:'10px 20px', border:'none', background:'transparent', cursor:'pointer', fontSize:13, fontWeight:tab===tb.id?700:500, color:tab===tb.id?C.primary:C.textSec, borderBottom:tab===tb.id?`2px solid ${C.primary}`:'2px solid transparent', marginBottom:-2, fontFamily:'inherit' }}>
            {tb.label}
          </button>
        ))}
      </div>
    ...
    <div style={{ display:'flex', gap:20 }}>
      <div style={{ flex:1 }}>
        ...
        <div style={{ ...card, padding:20, marginBottom:16 }}>
          ...
          <div style={{ display:'flex', gap:8, marginTop:12 }}>
            <button type="button" onClick={()=>setShowAiWriter(true)} style={makeBtn('secondary',{gap:5,fontSize:12})}>
        ...
        <div style={{ ...card, padding:16, marginBottom:16 }}>
          <div style={{ fontSize:12, fontWeight:600, color:C.textSec, marginBottom:10 }}>{isRTL?'المنصات':'Platforms'}</div>
          <div style={{ display:'flex', gap:8, flexWrap:'wrap' }}>
            {PLATFORMS.map(p => {
              const sel = selectedPlatforms.includes(p.id)
              return (
                <button type="button" key={p.id} onClick={()=>togglePlatform(p.id)} style={{ display:'flex', alignItems:'center', gap:6, padding:'8px 14px', borderRadius:8, border: sel?`2px solid ${p.color}`:`1px solid ${C.border}`, background: sel?`${p.color}10`:C.white, color: sel?p.color:C.textSec, fontSize:12, fontWeight:600, cursor:'pointer', fontFamily:'inherit', minHeight:36 }}>
          ...
          {text && selectedPlatforms.length > 0 && (
            <div style={{ display:'flex', gap:12, marginTop:12, flexWrap:'wrap' }}>
              {selectedPlatforms.map(pid => {
                ...
                return <span key={pid} style={{ fontSize:10, color: over?'#CF222E':C.textMuted }}>{p.name}: {text.length}/{p.maxChars}{over?' ⚠️':' ✓'}</span>
              })}
            </div>
          )}
        </div>
        ...
        <button type="button" onClick={handlePublish} disabled={!text.trim()||selectedPlatforms.length===0} style={{ ...makeBtn('primary',{gap:6, width:'100%', justifyContent:'center', padding:'12px', fontSize:15}), opacity:(!text.trim()||selectedPlatforms.length===0)?.5:1 }}>
      ...
      <div style={{ width:320, flexShrink:0 }}>
        <div style={{ ...card, padding:16 }}>
          ...
          {!text ? <p style={{ fontSize:12, color:C.textMuted, textAlign:'center', padding:20 }}>{isRTL?'اكتب منشورك لمعاينته':'Write your post to see preview'}</p> : selectedPlatforms.map(pid => {
            const p = PLATFORMS.find(x=>x.id===pid)
            return (
              <div key={pid} style={{ marginBottom:12, borderRadius:10, border:`1px solid ${C.border}`, overflow:'hidden' }}>
                <div style={{ padding:'8px 12px', background:`${p.color}10`, display:'flex', alignItems:'center', gap:6 }}>
                ...
                <div style={{ padding:12 }}>
                  <div style={{ display:'flex', alignItems:'center', gap:8, marginBottom:8 }}>
                    <div style={{ width:28, height:28, borderRadius:'50%', background:C.primaryBg, color:C.primary, display:'flex', alignItems:'center', justifyContent:'center', fontSize:11, fontWeight:700 }}>{(orgSettings?.name||'V').charAt(0)}</div>
                    <div><div style={{ fontSize:11, fontWeight:600, color:C.text }}>{orgSettings?.name||'Velo'}</div><div style={{ fontSize:9, color:C.textMuted }}>Just now</div></div>
                  </div>
                  <p style={{ fontSize:12, color:C.text, lineHeight:1.5, margin:0, whiteSpace:'pre-wrap', wordBreak:'break-word' }}>{text.length > p.maxChars ? text.slice(0,p.maxChars)+'...' : text}</p>
                </div>
              </div>
            )
          })}
      ...
      {showAiWriter && (
        <Modal onClose={()=>setShowAiWriter(false)} dir={dir} width={440}>
          <h3 style={{ fontSize:16, fontWeight:700, color:C.text, margin:'0 0 12px' }}>{isRTL?'مساعد الكتابة الذكي':'AI Writing Assistant'}</h3>
          ...
          <div style={{ display:'flex', gap:8, justifyContent:'flex-end' }}>
            ...
            <button type="button" onClick={generateAi} disabled={!aiPrompt.trim()} style={makeBtn('primary',{gap:5})}>{isRTL?'توليد':'Generate'}</button>
          </div>
        </Modal>
      )}
    </div>
  ...
  return (
    <div>
      <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:16 }}>
        <button type="button" onClick={()=>setCurrentDate(new Date(year,month-1,1))} style={{border:'none',background:'transparent',cursor:'pointer',color:C.textSec,display:'flex'}}>{isRTL?Icons.chevronRight(20):Icons.chevronLeft(20)}</button>
        <span style={{ fontSize:16, fontWeight:700, color:C.text }}>{monthName}</span>
        <button type="button" onClick={()=>setCurrentDate(new Date(year,month+1,1))} style={{border:'none',background:'transparent',cursor:'pointer',color:C.textSec,display:'flex'}}>{isRTL?Icons.chevronLeft(20):Icons.chevronRight(20)}</button>
      </div>
      <div style={{ ...card, overflow:'hidden' }}>
        ...
        <div style={{ display:'grid', gridTemplateColumns:'repeat(7,1fr)' }}>
          {cells.map((day,i) => {
            if(!day) return <div key={i} style={{ minHeight:70, background:C.bg, borderBottom:`1px solid ${C.border}`, borderRight:`1px solid ${C.border}` }} />
            const dayPosts = getPostsForDate(day)
            return (
              <div key={i} onClick={()=>setSelectedDay(dayPosts.length?day:null)} style={{ minHeight:70, padding:4, borderBottom:`1px solid ${C.border}`, borderRight:`1px solid ${C.border}`, cursor: dayPosts.length?'pointer':'default' }}>
                <div style={{ fontSize:12, fontWeight:500, color:C.text, marginBottom:4 }}>{day}</div>
                {dayPosts.slice(0,2).map(p => (
                  <div key={p.id} style={{ fontSize:9, padding:'2px 4px', marginBottom:2, borderRadius:3, background: p.status==='published'?'#DAFBE1':'#DDF4FF', color: p.status==='published'?'#1A7F37':'#0969DA', fontWeight:600, overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap' }}>
                    {p.platforms.map(pid=>PLATFORMS.find(x=>x.id===pid)?.icon).join(' ')} {p.text.slice(0,20)}
                  </div>
                ))}
                {dayPosts.length>2 && <div style={{ fontSize:8, color:C.textMuted }}>+{dayPosts.length-2}</div>}
              </div>
            )
          })}
        </div>
      ...
      {selectedDay && (
        <Modal onClose={()=>setSelectedDay(null)} dir={dir} width={440}>
          <h3 style={{ fontSize:16, fontWeight:700, color:C.text, margin:'0 0 16px' }}>{isRTL?'منشورات اليوم':'Posts for this day'}</h3>
          {getPostsForDate(selectedDay).map(p => (
            <div key={p.id} style={{ padding:12, borderRadius:8, border:`1px solid ${C.border}`, marginBottom:8 }}>
              <div style={{ display:'flex', gap:6, marginBottom:6 }}>{p.platforms.map(pid=>{const pl=PLATFORMS.find(x=>x.id===pid);return <span key={pid} style={{ fontSize:10, padding:'2px 6px', borderRadius:4, background:`${pl?.color}15`, color:pl?.color, fontWeight:600 }}>{pl?.name}</span>})}</div>
              <p style={{ fontSize:12, color:C.text, lineHeight:1.5, margin:0 }}>{p.text}</p>
              <div style={{ fontSize:10, color:C.textMuted, marginTop:6 }}>{p.scheduledAt?.replace('T',' ')} — {p.status==='published'?(isRTL?'منشور':'Published'):(isRTL?'مجدول':'Scheduled')}</div>
            </div>
          ))}
        </Modal>
      )}
...
function AnalyticsTab({ lang, dir, isRTL }) {
  ...
  const metrics = [
    ...
    { label: isRTL?'الانطباعات':'Impressions', value: d.impressions.toLocaleString(), color:'#8250DF', bg:'#FBEFFF' },
    { label: isRTL?'التفاعل':'Engagement', value: d.engagement+'%', color:'#1A7F37', bg:'#DAFBE1' },
    { label: isRTL?'النقرات':'Link Clicks', value: d.clicks.toLocaleString(), color:'#D29922', bg:'#FFF8C5' },
    { label: isRTL?'متابعون جدد':'New Followers', value: '+'+d.followers, color:'#CF222E', bg:'#FFEBE9' },
  ]
  ...
  return (
    <div>
      <div style={{ display:'grid', gridTemplateColumns:'repeat(5,1fr)', gap:12, marginBottom:24 }}>
        {metrics.map((m,i)=>(
          <div key={i} style={{ ...card, padding:16, textAlign:'center' }}>
            <div style={{ fontSize:10, color:C.textMuted, fontWeight:600, marginBottom:6 }}>{m.label}</div>
      ...
      <div style={{ ...card, padding:20, marginBottom:20 }}>
        <h3 style={{ fontSize:14, fontWeight:600, color:C.text, margin:'0 0 16px' }}>{isRTL?'الوصول — آخر 30 يوم':'Reach — Last 30 Days'}</h3>
      ...
      <div style={{ ...card, padding:20 }}>
        <h3 style={{ fontSize:14, fontWeight:600, color:C.text, margin:'0 0 16px' }}>{isRTL?'أفضل المنشورات':'Top Performing Posts'}</h3>
        {d.topPosts.map(p => {
          ...
          return (
            <div key={p.id} style={{ display:'flex', alignItems:'center', gap:12, padding:'12px 0', borderBottom:`1px solid ${C.border}` }}>
              ...
              <div style={{ flex:1, minWidth:0 }}><div style={{ fontSize:12, color:C.text, overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap' }}>{p.text}</div></div>
              <div style={{ display:'flex', gap:16, fontSize:11, color:C.textSec, flexShrink:0 }}>
...
function AccountsTab({ lang, dir, isRTL }) {
  const accounts = [
    ...
    { id:'tw', name:'X (Twitter)', platform:'twitter', icon:'𝕏', color:'#1F2328', connected:false },
  ...
  return (
    <div style={{ display:'flex', flexDirection:'column', gap:12 }}>
      {accounts.map(acc => (
        <div key={acc.id} style={{ ...card, padding:20, display:'flex', alignItems:'center', gap:16 }}>
          <div style={{ width:44, height:44, borderRadius:12, background:`${acc.color}15`, color:acc.color, display:'flex', alignItems:'center', justifyContent:'center', fontSize:20, fontWeight:700, flexShrink:0 }}>{acc.icon}</div>
          <div style={{ flex:1 }}>
            <div style={{ fontSize:14, fontWeight:600, color:C.text }}>{acc.name}</div>
            {acc.connected ? (
              <div style={{ fontSize:12, color:C.textSec, marginTop:2 }}>{acc.accountName} &middot; {acc.followers} {isRTL?'متابع':'followers'} &middot; {isRTL?'آخر نشر:':'Last post:'} {acc.lastPost}</div>
            ) : (
              <div style={{ fontSize:12, color:C.textMuted, marginTop:2 }}>{isRTL?'غير متصل':'Not connected'}</div>
            )}
          </div>
          {acc.connected ? (
            <span style={{ fontSize:11, fontWeight:600, padding:'4px 10px', borderRadius:6, background:'#DAFBE1', color:'#1A7F37' }}>{isRTL?'متصل':'Connected'}</span>
          ) : (
