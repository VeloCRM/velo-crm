-- ═══════════════════════════════════════════════════════════════════════════
-- Velo CRM — Supabase Database Schema
...
-- ─── Conversations ──────────────────────────────────────────────────────────
-- (Created before tickets because tickets references conversations)
...
CREATE TABLE tickets (
  ...
  user_id         UUID NOT NULL REFERENCES auth.users(id) ON DELETE CASCADE,
  contact_id      UUID REFERENCES contacts(id) ON DELETE SET NULL,
  ticket_number   TEXT UNIQUE NOT NULL,  -- e.g. 'VLO-001'
...
-- Ticket comments: users can manage comments on their tickets
