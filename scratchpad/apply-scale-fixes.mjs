#!/usr/bin/env node
/**
 * apply-scale-fixes.mjs — REAL STAGING APPLY of the scale fixes. Runs two files
 * VERBATIM, in order: scripts/scale-indexes-migration.sql FIRST (the 8 indexes),
 * then scripts/patient-outstanding-balances-view.sql (the security_invoker view +
 * REVOKE/GRANT). Indexes must exist before the view so its active-row anti-join is
 * index-backed. Each file has no own BEGIN/COMMIT, so each runs in node-pg's implicit
 * transaction and COMMITS on success; the SELECT-only VERIFY then reads committed state.
 *
 * ⚠️ THIS PERSISTS TO STAGING. Not a dry-run. Run scratchpad/dryrun-scale-fixes.mjs
 *    FIRST and confirm all assertions pass.
 *
 * Guards:
 ...
 *   1. all 8 scale indexes exist.
 *   2. patient_outstanding_balances view exists + reloptions security_invoker on.
 ...
 *   node scratchpad/apply-scale-fixes.mjs --confirm
 *
...
if (!process.argv.includes('--confirm')) {
  console.error('REFUSING TO RUN: this APPLIES + COMMITS the scale indexes + view to STAGING.')
  console.error('Re-run with the explicit flag once the dry-run is clean:')
  console.error('  node scratchpad/apply-scale-fixes.mjs --confirm')
  process.exit(1)
}
...
try {
  ...
  console.log(`\n⚠️  STAGING APPLY — scale indexes + patient_outstanding_balances view (${u.hostname})\n${'='.repeat(74)}`)

  console.log('1/2 running scale-indexes-migration.sql VERBATIM (implicit transaction → real commit)…')
  await client.query(indexSql)
  appliedIndexes = true
  console.log('✓ indexes COMMITTED to staging.\n')
  ...
  runError = e
  // A mid-batch failure aborts that file's implicit transaction (never committed).
...
if (appliedIndexes && appliedView) {
  console.log(`APPLIED to staging (indexes + view). ${results.length - failed.length}/${results.length} VERIFY checks passed.`)
  ...
  console.log('NOT APPLIED — the index SQL errored before commit; staging is UNCHANGED.')
}
if (failed.length) console.log('FAILED:\n  ' + failed.map((f) => f.name).join('\n  '))
if (runError) console.log(`\nError: ${runError.stack || runError.message}`)
console.log('\nRun command (PowerShell):')
console.log("  $env:STAGING_DB_URL='postgresql://postgres:<STAGING_DB_PASSWORD>@db.dujnbboyeugrisgewnqu.supabase.co:5432/postgres'")
console.log('  node scratchpad/apply-scale-fixes.mjs --confirm')
