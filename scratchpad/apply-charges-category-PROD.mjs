#!/usr/bin/env node
/**
 * apply-charges-category-PROD.mjs — REAL PRODUCTION APPLY of the Slice 4b category
 ...
 *      (scratchpad/dryrun-charges-category-PROD.mjs) FIRST and confirm it is clean.
 *
 * Guards (INVERTED — same as apply-billing-slice1-PROD.mjs):
 ...
 * and skip VERIFY). The migration is idempotent (ADD COLUMN IF NOT EXISTS / guarded
 * DO block for the CHECK / DROP NOT NULL is a no-op when already nullable), so a
 ...
 * (No functional inserts — this is a real commit; we do not persist test rows.)
 *
 * ── Run (PowerShell) ─────────────────────────────────────────────────────────
 *   $env:PROD_DB_URL='postgresql://postgres:<PROD_DB_PASSWORD>@db.aajwuwjxpmmqcwhiynla.supabase.co:5432/postgres'
 *   node scratchpad/apply-charges-category-PROD.mjs --confirm
 *
...
if (!process.argv.includes('--confirm')) {
  console.error('REFUSING TO RUN: this APPLIES + COMMITS the Slice 4b category migration to PRODUCTION.')
  console.error('Re-run with the explicit flag once the PROD dry-run is clean:')
  console.error('  node scratchpad/apply-charges-category-PROD.mjs --confirm')
  process.exit(1)
}
...
try {
  ...
  console.log(`\n⚠️  PRODUCTION APPLY — Slice 4b charges.category migration (${u.hostname})\n${'='.repeat(70)}`)
